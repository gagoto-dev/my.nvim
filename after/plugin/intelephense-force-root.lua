...
do
    local doc = configs.intelephense and configs.intelephense.document_config
    if doc and doc.default_config then
        doc.default_config.root_dir = function(_) return ROOT end
...
-- 3) Asegura un setup explícito (por si nadie lo llama)
lspconfig.intelephense.setup({
    -- si usas 'capabilities' globales, descomenta:
